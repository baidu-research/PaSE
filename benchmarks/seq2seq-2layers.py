...
def Seq2seq(src, tgt, params, src_vocab_size, tgt_vocab_size, args):
    ...
    if strategy == 0 or strategy == 2:
        ...
        def MeshXToMeshY(x, mesh_x, mesh_y, shape=None, name='meshx_to_meshy'):
            assert x.mesh == mesh_x
            if x.mesh != meshes[2]:
                assert all(name != 'axis0' for name in x.shape.dimension_names)
                if mesh_y == meshes[2]:
                    shape = shape or ['axis0' if s=='axis1' else s for s in
                            x.shape.dimension_names]
                else:
                    shape = shape or x.shape.dimension_names
            else:
                shape = shape or ['axis1' if s=='axis0' else s for s in
                        x.shape.dimension_names]

            return ReplaceMeshWithDuplicates(x, mesh_y, shape, name=name)

        Mesh0ToMesh1 = partial(MeshXToMeshY, mesh_x=meshes[0], mesh_y=meshes[1])
        Mesh1ToMesh0 = partial(MeshXToMeshY, mesh_x=meshes[1], mesh_y=meshes[0])
        Mesh0ToMesh2 = partial(MeshXToMeshY, mesh_x=meshes[0], mesh_y=meshes[2])
        Mesh1ToMesh2 = partial(MeshXToMeshY, mesh_x=meshes[1], mesh_y=meshes[2])
        Mesh2ToMesh0 = partial(MeshXToMeshY, mesh_x=meshes[2], mesh_y=meshes[0])
        Mesh2ToMesh1 = partial(MeshXToMeshY, mesh_x=meshes[2], mesh_y=meshes[1])
    ...
    class LSTMCell():
        ...
        def __call__(self, x, context=None, reshape_output=False):
            ...
            if context is not None:
                if strategy == 1:
                    if context.mesh != self.mesh:
                        if self.mesh == meshes[0]:
                            context = Mesh2ToMesh0(context,
                                    name='replace_context_mesh')
                        else:
                            context = Mesh2ToMesh1(context,
                                    name='replace_context_mesh')

                    if context.shape != self.shape:
                        context = mtf.reshape(context, self.shape)
                concat_tsrs += context,
    ...
    class RNNStack():
        ...
        def Unidirectional(self, xs):
            ...
            for x in xs:
                ...
                with tf.variable_scope(f'{self.name}_layer_1'):
                    x = self.layers[1](x, self.context)
        ...
        def Bidirectional(self, xs):
            raise NotImplementedError('Bi-directional RNN not implemented yet.')
    ...
    def RNN(xs, output_dims, hs=None, bidirectional=False, attention_fn=None,
        ...
        if strategy == 0:
            ...
            if attention_fn is None: # Encoder
                ...
                repl_fns = [None, Mesh0ToMesh1]
            else: # Decoder
                layer_meshes = [meshes[1], meshes[0]]
                shape = xs[0].shape
                assert shape[1].name == 'axis0'
                new_shape = utils.RenameDim(shape, 1, 'axis1')
                shapes = [shape, new_shape]
                repl_fns = [None, Mesh1ToMesh0]
    ...
    with tf.variable_scope('encoder'):
        ...
        enc_out, enc_hs = RNN(embed, enc_rnn_out_dims, bidirectional=False,
                name='encoder_rnn')
    ...
    with tf.variable_scope('decoder'):
        ...
        if strategy == 1:
            ...
            enc_hs[1] = Mesh1ToMesh0(mtf.rename_dimension(enc_hs[1], 'axis0',
                'axis1'))
            assert enc_hs[0].mesh == meshes[1] and enc_hs[1].mesh == meshes[0]
        ...
        def Attention(h):
            ...
            # Softmax
            if strategy == 1:
                score = Mesh1ToMesh2(mtf.rename_dimension(score, 'axis0',
                    RandName()))
            score = mtf.softmax(score, score.shape[-1], name=f'attn_score_softmax')
            if strategy == 1:
                score = mtf.rename_dimension(score, score.shape[-1].name,
                        enc_out.shape[1].name)

            # Einsum
            assert score.mesh == enc_out.mesh
            assert score.shape[1] == enc_out.shape[1]
            context = mtf.einsum([score, enc_out],
                    reduced_dims=[score.shape[1]], name='attn_context_einsum')
            if strategy == 1:
                context = mtf.rename_dimension(context, context.shape[-1].name,
                        'axis0')
            return context
