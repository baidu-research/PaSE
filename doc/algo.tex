...
\begin{algorithmic}[1]
	...
	\While{$U \not = \emptyset$}
		...
		all the common node configurations are the same. Refer
		...
		\parbox[t]{.7\textwidth}{
				...
				\land~\\\Call{IsCompatible}{v_{min}, S_i, S_i} = \top)
				\land~\\i \not = j \implies \Call{IsCompatible}{v_{min}, S_i, S_j} =
			\bot)
		\}$}
		\Statex
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\Procedure{IsCompatible}{$v, S_1, S_2$}
			\ForAll{$s_1 \in S_1$}
				\ForAll{$s_2 \in S_2$}
	...
	\begin{algorithmic}[1]
		...
		\ForAll{$v \in V$}
			...
			\ForAll{$S \in \strategy$}
				\ForAll{$T \in v.tbl$}
					\If{$\Call{IsCompatible}{S, T} = \top$}
						\State $S' \leftarrow S \cup T$
						\State $\strategy' \leftarrow \strategy' \cup \set{S'}$
					\EndIf
				\EndFor
			\EndFor
